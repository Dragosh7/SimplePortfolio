...
monthsBetween dA dB =
    let
        (Date a) = dA
        (Date b) = dB
        (yearA, monthA) = (a.year, a.month)
        (yearB, monthB) = (b.year, b.month)
    in
        case (monthA, monthB) of
                (Just mA, Just mB) ->
                    Just <| abs <| (yearB - yearA) * 12 + monthToInt mB - monthToInt mA

                (Nothing, Nothing) ->
                    Just <| abs <| (yearB - yearA) * 12
...
compare (Date d1) (Date d2) =
    let
        (year1, month1) = (d1.year, d1.month)
        (year2, month2) = (d2.year, d2.month)

        compareMonths : Maybe Month -> Maybe Month -> Order
        compareMonths maybeMonthA maybeMonthB =
            case (maybeMonthA, maybeMonthB) of
                (Just monthA, Just monthB) ->
                    compareMonth monthA monthB  
                ...
                (Nothing, Nothing) ->
                    EQ
    in Model.Util.chainCompare (compareMonths month1 month2) (Basics.compare year1 year2)
    --if (year1 < year2) then LT else if (year1 > year2) then GT else compareMonths month1 month2
...
view (Date d) =
    div [class "date"]
        [case d.month of
        Just m -> text (monthToString m ++ " " ++ String.fromInt d.year)

        Nothing -> text (String.fromInt d.year) 
    ]
